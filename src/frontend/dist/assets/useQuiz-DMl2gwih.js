...
const AnswerSubmission = Record({ "answers": Vec(Nat) });
...
Service({
  "getQuestions": Func([], [Vec(Question)], ["query"]),
  "submitAnswers": Func([AnswerSubmission], [QuizResult], [])
});
const idlFactory = ({ IDL: IDL2 }) => {
  ...
  const AnswerSubmission2 = IDL2.Record({ "answers": IDL2.Vec(IDL2.Nat) });
  ...
  return IDL2.Service({
    "getQuestions": IDL2.Func([], [IDL2.Vec(Question2)], ["query"]),
    "submitAnswers": IDL2.Func([AnswerSubmission2], [QuizResult2], [])
  });
};
class Backend {
  ...
  return useMutation({
    mutationFn: async ({ answers }) => {
      ...
      const correctCount = answers.reduce((acc, ans, idx) => {
        var _a2;
        return acc + (ans === ((_a2 = questions[idx]) == null ? void 0 : _a2.correctIndex) ? 1 : 0);
      }, 0);
...
export {
  useSubmitAnswers as a,
  useGetQuestions as u
};
